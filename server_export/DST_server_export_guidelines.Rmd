...
Our interpretation of the regulatory requirements are intentionally strict to avoid [severe con-sequences for the individual and the research group](https://www.dst.dk/da/TilSalg/data-til-forskning/regler-og-datasikkerhed/sanktionsregler).

The process of exporting from the DST servers is visualized in the flowchart below.
...
1. Reverse engineering small counts should also not be possible across multiple tables that may be exported throughout the course of your project.
1. Use intuitive variable names, especially for variables that are key to our review. Use for example `N_subjects` or `N_persons` rather than `n`, `N`, `Count`, `Cases` etc. If your analysis is event-based such as most survival-type analyses you should also have a column named `N_events` and potentially `N_subjects_with_events` if subjects can have more than one event.
